...
import { Construction, Plus, Search, Download, Edit2, Trash2, TrendingUp, AlertCircle, CheckCircle2, DollarSign } from 'lucide-react';
...
export default function InfrastructureMonitoringPage() {
  ...
  // Firebase listener for real-time updates (read-only for users)
  useEffect(() => {
    console.log('🔥 [USER] Setting up Firebase listener for infrastructure projects...');
    console.log('🔥 [USER] Firebase DB instance:', db);
    
    const q = query(collection(db, 'infrastructure_projects'), orderBy('createdAt', 'desc'));
    const unsubscribe = onSnapshot(q, (snapshot) => {
      console.log('🔥 [USER] Firebase snapshot received:', snapshot.docs.length, 'documents');
      ...
      console.log('🔥 [USER] Final projects data:', projectsData);
      setProjects(projectsData);
      setLoading(false);
    }, (error) => {
      console.error('🔥 [USER] Error fetching projects:', error);
    ...
    return (
      <div className="space-y-5">
        <PageHeader
          ...
          description="View infrastructure projects across the province"
          icon={Construction}
        />
  ...
  return (
    <div className="space-y-5">
      ...
      {/* Add Project Dialog - Same as Admin */}
      ...
      <Dialog open={isEditDialogOpen} onOpenChange={setIsEditDialogOpen}>
        <DialogContent className="max-w-4xl max-h-[90vh] overflow-y-auto">
          ...
          <div className="grid gap-4 py-4">
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
              <div className="space-y-2">
                <Label htmlFor="edit-paid">Payment Status</Label>
                <Select value={formData.paid} onValueChange={value => setFormData({ ...formData, paid: value as any })}>
                  <SelectTrigger>
                    <SelectValue placeholder="Select status" />
                  </SelectTrigger>
